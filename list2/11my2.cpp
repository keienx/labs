...
void insert(List& list, string& value, int& index) {
    Point* new_point = new Point;
    new_point -> value = value;
    if (list.head == nullptr) { 
        new_point->prev = nullptr;
...
void showmenu(){
    ...
    cout << "6. Вставить элемент перед заданным индексом\n";
...
void menu(){
    string str;
    int choice = 1, size, quantity, index;
    while (choice != -1){
        system("pause");
        system("cls");
        showmenu();
